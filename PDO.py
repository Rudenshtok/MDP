...
def step(env, policy_model, value_function_model, cost_value_function_model, cost_value_function_model2, gamma,
         ...
         limit, limit2, lagrange1, lagrange2, eta):

    #max_kl controls the size of the ball.
    # Generate rollout
    ...
    for batch_num in range(num_batches):
      ...
      if policy_gradient.nonzero().size()[0]:
          ...
          lagrange1 = max(lagrange1 + eta * (limit - total_cost), 0.0) # previous 0.0002 looks good, 0.0005 still ok
          lagrange2 = max(lagrange2 + eta * (limit2 - total_cost2), 0.0)
...
def pdo(eta):
    ...
    for iteration in range(STEP):
        result, violation, violation2, lagrange1, lagrange2 = step(env, policy_model, value_function_model, cost_value_function_model, cost_value_function_model2, gamma,
                                 episodes, length, batch_size, max_kl, cg_iters, residual_tol, cg_damping, ent_coeff,
                                 penality, penality2, limit, limit2, lagrange1, lagrange2, eta)
        ...
        if (iteration + 1) % 50 == 0:
            print("step:", iteration + 1, "test result:", result, "test violation:", -1*violation, "test violation2:", -1*violation2)
