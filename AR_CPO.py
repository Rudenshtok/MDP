...
def step(env, policy_model, value_function_model, cost_value_function_model, cost_value_function_model2, gamma,
         ...
         limit, limit2, lower_lagrange1, lower_lagrange2, mu):
    # Generate rollout
    ...
    for batch_num in range(num_batches):
        ...
        lagrange_advantage = normalize(advantage, cost_advantage, cost_advantage2, lower_lagrange1, lower_lagrange2)
        ...
        if policy_gradient.nonzero().size()[0]:
            ...
            # gradient_lagrange1 = total_cost - limit + mu * lower_lagrange1

            # gradient_lagrange2 = total_cost2 - limit2 + mu * lower_lagrange2
            # lagrange1 = max(lagrange1 - 0.0005 * gradient_lagrange1, 0.0)  # previous 0.0002 looks good, 0.0005 still ok
            # lagrange2 = max(lagrange2 - 0.0005 * gradient_lagrange2, 0.0)
            ...
            #   if ev_after < ev_before or np.abs(ev_after) < 1e-4: #redudent
    ...
    return total_reward, total_cost, total_cost2, gradient_lagrange1, gradient_lagrange2


def arcpo(inner_loop, max_kl, eta, s, stopping_time):
    ...
    for iteration in range(out_loop):
        ...
        lower_lagrange1 = (1 - q) * upper_lagrange1 + q * lagrange1
        lower_lagrange2 = (1 - q) * upper_lagrange2 + q * lagrange2

        for inner_iteration in range(inner_loop):

            result, violation, violation2, gradient_lagrange1, gradient_lagrange2 = step(env, policy_model,
                                                                                         value_function_model,
                                                                                         cost_value_function_model,
                                                                                         cost_value_function_model2,
                                                                                         gamma,
                                                                                         episodes, length, batch_size,
                                                                                         max_kl, cg_iters, residual_tol,
                                                                                         cg_damping, ent_coeff,
                                                                                         penality, penality2, limit,
                                                                                         limit2, lower_lagrange1,
                                                                                         lower_lagrange2, mu)
            ...
            running_iteration = iteration * inner_loop + inner_iteration

            result_avg = (running_iteration / (running_iteration + 1)) * result_avg + (
                        1 / (running_iteration + 1)) * result
            violation_avg = (running_iteration / (running_iteration + 1)) * violation_avg + (
                        1 / (running_iteration + 1)) * (-1 * violation)
            violation2_avg = (running_iteration / (running_iteration + 1)) * violation2_avg + (
                        1 / (running_iteration + 1)) * (-1 * violation2)

            results_avg.append(result_avg)
            violations_avg.append(violation_avg)
            violations2_avg.append(violation2_avg)

            iterations.append(running_iteration * episodes)

            lower_lagranges1.append(lower_lagrange1)
            lower_lagranges2.append(lower_lagrange2)
            lagranges1.append(lagrange1)
            lagranges2.append(lagrange2)
            upper_lagranges1.append(upper_lagrange1)
            upper_lagranges2.append(upper_lagrange2)
            if (running_iteration + 1) % 50 == 0:
                print("step:", running_iteration + 1, "test result:", result, "test violation:", -1 * violation,
                      "test violation2:", -1 * violation2)

            print('lagrange1, lagrange2 is', lagrange1, lagrange2)
            print('lower_lagrange1, lower_lagrange2 is', lower_lagrange1, lower_lagrange2)

        lagrange1 = max(lagrange1 - eta * gradient_lagrange1, 0.0)  # eta =  previous 0.0002 looks good, 0.0005 still ok
        lagrange2 = max(lagrange2 - eta * gradient_lagrange2, 0.0)
        ...
        # alphas.append(alpha)
        upper_lagrange1 = (1 - alpha) * upper_lagrange1 + alpha * lagrange1
        upper_lagrange2 = (1 - alpha) * upper_lagrange2 + alpha * lagrange2

    return results, violations, violations2, lagranges1, lagranges2, lower_lagranges1, lower_lagranges2, upper_lagranges1, upper_lagranges2